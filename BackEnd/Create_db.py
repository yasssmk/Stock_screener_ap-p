...
def create_stock_data_db(stock_list):
    ...
    for symbol in symbols:
        try:
            ...
            last_eps = Stock_Data.get_eps(symbol)
            av_eps = Stock_Data.get_3years_av_eps(symbol)
            result_df = Stock_Data.get_yearly_growth(symbol)
            eps_growth = Stock_Data.calculate_eps_growth(result_df)
            median_eps_growth = Stock_Data.calculate_median_eps_growth(result_df)
            years_data = Stock_Data.get_years_of_data(symbol)
            last_growth = Stock_Data.get_last_growth(symbol)
            dte = Stock_Data.debt_to_equity_ratio(symbol)
            nwc = Stock_Data.get_nwc(symbol)
            roe = Stock_Data.get_roe(symbol)
            fcf = Stock_Data.get_fcf(symbol)
            ...
            Errors_logging.functions_error_log("create_stock_data_db", e, Errors_logging.log_name_rundb, symbol=symbol)
            continue
    ...
    try:
        ...
        Errors_logging.functions_error_log("create_stock_data_db", e, Errors_logging.log_name_rundb)
        #print(f'Error: {e}')
...
def remove_outliers(data):
    try:
        ...
        Errors_logging.functions_error_log("remove_outliers", e, Errors_logging.log_name_rundb)
...
def calculate_industry_stat():
    ...
    try:
        ...
        Errors_logging.functions_error_log("calculate_industry_stat", e, Errors_logging.log_name_rundb)
        #print(f'Error: {e}')
...
def calculate_sector_stat():
    ...
    try:
        ...
        Errors_logging.functions_error_log("calculate_sector_stat", e, Errors_logging.log_name_rundb)
...
def add_percentile(dataframe):
    try:
        ...
        Errors_logging.functions_error_log("add_percentile", e, Errors_logging.log_name_rundb)
...
def custom_ranking(symbol):
    try:
        ...
        Errors_logging.functions_error_log("custom_ranking", e, Errors_logging.log_name_rundb, symbol=symbol)
        return None
...
def is_outperformer(symbol):
    try:
        ...
        Errors_logging.functions_error_log("is_outperformer", e, Errors_logging.log_name_rundb, symbol=symbol)
    ...
    try:
        ...
        for index, row in data_df.iterrows():
            try:
                ...
                symbol = row['Symbol']
                Errors_logging.functions_error_log("update_databases", e, Errors_logging.log_name_rundb, symbol=symbol)
                continue
        ...
        Sending_Email.job_done_email("Update Data Base", error_log_path)

    except Exception as e:
        Errors_logging.functions_error_log("update_databases", e, Errors_logging.log_name_rundb)
        error_log_path = './stocks_data_csv/errors_logs/Create_update_selection'
        Sending_Email.db_error_email(e, "Data Base", error_log_path)
