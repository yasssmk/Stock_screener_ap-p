...
def get_data(symbol):
    try:
        ...
        Errors_logging.functions_error_log("get_data", e, Errors_logging.log_name_weekly_signals,
                                           symbol=symbol)
...
def get_monthly_data(symbol):
    try:
        ...
        Errors_logging.functions_error_log("get_monthly_data", e, Errors_logging.log_name_monthly_signals,
                                           symbol=symbol)
...
def get_lt_trend(stock_data, date):
    try:
        ...
        Errors_logging.functions_error_log("get_lt_trend", e, Errors_logging.log_name_weekly_signals)
...
def get_macd_trend(stock_data, date):
    try:
        ...
        Errors_logging.functions_error_log("get_macd_trend", e, Errors_logging.log_name_weekly_signals)
...
def spot_weekly_signals(file_data, date, lt_trend, stock_data_weekly, stock_data_monthly):
    ...
    # Indicators
    bb_low_serie = Stock_Data.bb_low_indicator(stock_data_weekly)
    bb_low = bb_low_serie[date]
    bb_high_serie = Stock_Data.bb_high_indicator(stock_data_weekly)
...
def spot_monthly_signals(file_data, date, lt_trend, stock_data_monthly):
    ...
    # Indicators
    bb_low_serie = Stock_Data.bb_low_indicator(stock_data_monthly)
    bb_low = bb_low_serie[date]
    ...
    try:
        ...
        if action == 'Buy':
            ...
            for index, row in portfolio_df.loc[portfolio_df['Symbol'] == symbol].iterrows():
                buying_price = float(row['Buying Price'].replace('$', ''))
                selling_price_str = Stock_Data.get_stock_price(symbol)
                selling_price = float(selling_price_str.replace('$', ''))
                profit = selling_price - buying_price
                yield_ = profit / buying_price
...
def update_portfolio_details():
    try:
        ...
        for index, row in portfolio_df.iterrows():
            current_price_str = Stock_Data.get_stock_price(row['Symbol'])
        ...
        if not os.path.exists(path_file):
            ...
            for date in stock_data.index:
                try:
                    ...
                    Errors_logging.functions_error_log("add_monthly_stock_data", e,
                                                       Errors_logging.log_name_monthly_signals,
                                                       symbol=symbol)
                    continue
            ...
            if macd_trend != last_row['MACD Trend'] or lt_trend != file_data.iloc[-1]['LT Trend']:
                ...
                file_data.loc[file_data.index[-1], 'Divergence spotted'] = spot_divergence
                if spot_divergence == 'Buy - Monthly Divergence':
                    Sending_Email.process_buy_signals(symbol, current_date)
                    update_portfolio(symbol, 'Buy', current_date)
                elif spot_divergence == 'Sell - Monthly divergence':
                    if symbol_held:
                        Sending_Email.process_sell_signals(symbol, current_date)
                ...
                try:
                    ...
                    if file_data.empty or macd_trend != file_data.iloc[-1]['MACD Trend'] or lt_trend != \
                        ...
                        try:
                            ...
                            Errors_logging.functions_error_log("add_weekly_stock_data", e,
                                                               Errors_logging.log_name_weekly_signals,
                                                               symbol=symbol)
